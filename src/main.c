...
float lattice_w[LATTICE_Q] = {4.0f / 9.0f,  1.0f / 9.0f,  1.0f / 9.0f,
...
void calc_flow_properties_from_boltzmann_local(float N[LATTICE_Q * NUMEL],
...
void calc_flow_properties_from_boltzmann(float N[LATTICE_Q * NUMEL],
                                         float rho[NUMEL], float u[NUMEL],
                                         float v[NUMEL],
                                         int offset_N[LATTICE_Q]) {
...
void collide_local(float N[LATTICE_Q * NUMEL], float tau, int index,
                   int offset_N[LATTICE_Q]) {
    float Neq[LATTICE_Q];
    float rho, u, v;
...
void init(float N[LATTICE_Q * NUMEL], float rho, float u, float v,
          int offset_N[LATTICE_Q]) {
    float Neq[LATTICE_Q];
    calc_equilibrium_from_flow_properties_local(rho, u, v, Neq);
...
void walls(int wall[], int nb_wall, float N[LATTICE_Q * NUMEL],
           int offset_N[LATTICE_Q]) {
    /* Parois */
...
void blows(int blow[], int nb_blow, float N[LATTICE_Q * NUMEL],
           int offset_N[LATTICE_Q], float p) {
    int dir[3] = {1, 5, 8};
...
void border_bc(float N[LATTICE_Q * NUMEL], int index, int dir[], int dir_fs[],
               int n_dir, int offset_N[LATTICE_Q]) {
    // dir: directions entrantes. Pour la condition limite considérées, indices
    // des vecteurs tels que c_i . n_bc > 0 (avec n_bc, vecteur normal pointant
    // vers l'intérieur du domaine). Par exemple pour une condtion limite en x =
    // 0, la normale entrante est e_x, donc dir = {1, 5, 8}. dir_fs: direction
    // de rebond des directions entrantes (ici rebond seulement dans la
    // direction normale pour laisser le fluide glisser dans la direction
    // tangentielle). n_dir: nombre de directions, pour un réseau D2Q9 toujours
    // égal à 3 sauf dans les coins où ne se propage que 1 direction.
...
int main() {
    ...
    for (int t = 0; t < 1000000; ++t) {
        ...
        if (t % (10 * period) == 0) {
            ...
            save(rho_1, u_1, v_1, t / (10 * period), SIZE_X, SIZE_Y);
        }
    }
